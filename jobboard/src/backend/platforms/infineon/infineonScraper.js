...
function parseLocation(locationText) {
  ...
  // Just use the location as city and default state to 'US'
  const city = locationText.trim();
  
  return { city: city, state: 'US' };
}

async function infineonScraper(searchQuery, maxPages = 20) {
  ...
  const allJobs = [];
  const baseUrl = "https://jobs.infineon.com/careers";

  try {
    // Construct the search URL
    const searchUrl = `${baseUrl}?query=${encodeURIComponent(searchQuery)}&location=united%20states&pid=563808961388750&domain=infineon.com&sort_by=relevance`;
    ...
    for (let i = 0; i < jobCards.length; i += batchSize) {
      ...
      for (let j = 0; j < batch.length; j++) {
        ...
        try {
          ...
          // Format the job data
          const formattedJob = {
            employer_name: "INFINEON",
            job_title: cleanJobTitle(jobData.title),
            job_city: city,
            job_state: state,
            job_posted_at: "Recently", // Infineon shows "Recently" as posting date
...
module.exports = infineonScraper;
